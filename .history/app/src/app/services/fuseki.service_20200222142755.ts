...
export class FusekiService {
    ...
    public getQuery(projectNo, query, reasoning?, queryType?){

        var endpoint = urljoin(this.settings, 'query');
    
        query = this._appendPrefixesToQuery(query);
    
        // Get query type if it has not been explicitly stated
        if(!queryType) queryType = this._getQueryType(query);
      ...
      public updateQuery(projectNo, query){
    
        const endpoint = urljoin(this.settings, 'update');
