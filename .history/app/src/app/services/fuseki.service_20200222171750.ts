...
export class FusekiService {
    ...
    private getAuth(settings){
        return `Basic ${window.btoa(settings.user + ':' + settings.password)}`;
    }

    public getQuery(query, endpoint?){

        if(!endpoint) endpoint = urljoin(this.settings.endpoint, 'query');
        ...
        var options: any = {};
        options.headers = {}
        options.headers['Authorization'] = this.getAuth(this.settings);
        ...
        if(queryType == 'construct' || queryType == 'describe'){
          return this._http.post(endpoint, `query=${query}`, options);
        }
        else{
          return this._http.post(endpoint, `query=${query}`, options)
            .pipe(
      ...
      public updateQuery(query, endpoint?){
        ...
        if(!endpoint) endpoint = urljoin(this.settings.endpoint, 'update');
